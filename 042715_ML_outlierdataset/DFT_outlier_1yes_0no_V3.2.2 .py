...
v3.2.2: #use minimum value instead of median
...
SMILES_list = []
with open(filename_CountBB) as f:
    for i,line in enumerate(f):
        values = line.strip().split(',')
        SMILES_list.append(values[0].strip())
f.close()
...
for y in args.y_flavor:
    ...
    list_yesno = []
    #############
    if filename_y != "DFT_HOMO_8_B3LYPt.csv":   ## specify the y file
        continue
    ##############
    ...
    df_1 = pd.DataFrame({"SMILES": list_SMILES,
                         "F18": list_yesno})
    df_1.to_csv("DFT_HOMO_BP86s_multi_1outlier0fitted_F18_V3.2.2.csv")
#    for SMILES in SMILES_list:
#        if SMILES in list_outliers_SMILES:
#            list_0_1_NA.append(1)
#        elif SMILES in list_fitted_SMILES:
...
#    num_z = (value_k - value_expected_k) / value_sigma
#
#df_1 = pd.DataFrame({"SMILES": SMILES_list,
##                     "F12": list_yesno[0],
##                     "F13": list_yesno[1],
##                     "F14": list_yesno[2],
##                     "F15": list_yesno[3],
...
#df_1.to_csv("DFT_HOMO_BP86s_1percent_1outlier0fitted_F18.csv")
                 
#####
end1 = time.time()
record_time("Part1_18_v2.0",start,end1,start)
#####
