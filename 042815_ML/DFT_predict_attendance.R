...
# df_bb <- read.csv('CountBB_0427.csv')
...
## Turn every integer bigger than 0 into 1
function_1<-function(x){
    if(x>0){num_1 <- 1}else if(x == 0){num_1 <- 0}
    return(num_1)
}
for(cname in names(df_bb)){
    if(class(df_bb[,cname]) != "integer"){next}
    for( i in 1:nrow(df_bb)){
        df_bb[i,cname] <- function_1(df_bb[i,cname])
    }
}
